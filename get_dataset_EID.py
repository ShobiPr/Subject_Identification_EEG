# -*- coding: utf-8 -*-
import scipy.io as spio
import numpy as np


def consecutive_index(data, _value, stepsize=1):
    data = np.where(data == _value)[0]
    result = np.split(data, np.where(np.diff(data) != stepsize)[0] + 1)
    return [[min(a), max(a)] for a in result]


def get_dataset_resting():
    '''
	Info from https://arxiv.org/pdf/1711.06149.pdf
	8 subjects (5 males and 3 females) # 3 trials per subject, # ech trial produce 7,000 EEG samples # = 168000
	'''
    # samples_subject = 21000 # samples per subject
    samples_trial = 7000  # samples per trial
    no_subjects = 8
    no_ch = 14
    sr = 128
    instance_len = sr * 2  # to create sub instances of 2 sec
    EEG_data = spio.loadmat('EID-M.mat', squeeze_me=True)['eeg_close_ubicomp_8sub'].T
    _labels = EEG_data[14, :]
    for subject in range(1, no_subjects + 1):
        s_index = consecutive_index(EEG_data[no_ch, :], subject)[0]
        for s_instance in range(s_index[0], s_index[1] + 1, samples_trial):
            _instance = EEG_data[:no_ch, s_instance:s_instance + samples_trial]
            max_instances = _instance.shape[1] / instance_len  # this is not necessary, but I added it just FYI
            for _i in range(0, 20):  # sub instances
                if _i < max_instances:
                    index_start, index_end = instance_len * _i, instance_len * (_i + 1)
                    sub_instance = _instance[:, index_start:index_end]
                    print('sub-instance {0} with shape {1} for subject {2}'.format(_i, sub_instance.shape, subject))
                # ch_fs_instances.append(get_features(sub_instance))
                # ch_tags_instances.append('subject_{0}'.format(subject))
    # return {"data": ch_fs_instances, "target": ch_tags_instances}
    return None


if __name__ == '__main__':
    dataset = get_dataset()
