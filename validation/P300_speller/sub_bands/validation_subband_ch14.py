...
logging.basicConfig(filename='Validation_sub_bands_P300_ch14_stat.log',
                    level=logging.INFO,
...
for ins in [10, 20, 40, 60]:
    logging.info("----------- stat ins: {0}---------".format(ins))
    for subject in range(1, 27):
        dataset = get_dataset(subject, ins)
        file_name = 'sub_bands_P300_ch14_stat_ins%2d.sav' % ins
