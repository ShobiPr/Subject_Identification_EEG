...
logging.basicConfig(filename='P300_EMD_ch56_energy_hht_60.log',
                    level=logging.INFO,
                    format='%(asctime)s %(levelname)-8s %(message)s')


def main():
    logging.info(" ***** EMD, CHANNELS:56, FEATURES: energy + hht***** \n")
    INSTANCES = [60]
    sr = 200
    ...
    for ins in INSTANCES:
            ...
            for session in range(1, 5):  # 4
                ...
                for f_instance in range(0, ins):
                    if f_instance not in [15, 21, 23, 45]:
                        ...
                        ch_fs_instances.append(get_features_emd(instance[:56, :], sr))  # CHANNELS: 56
        ...
        # saving the model
        model_name = 'P300_EMD2_ch56_energy_hht_ins%02d.sav' % ins
